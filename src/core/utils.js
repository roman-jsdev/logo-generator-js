...
export const validateInput = (node) => {
  ...
  value.length ? storage("title", value) : classList.add("input_danger");
...
export const createNewPage = (components, options) =>
  new AppRender("#app", {
...
export const responsiveTitle = () => {
  ...
  const cssClass = "basic__cards_title_font-size";
...
export const updateCounter = (key) =>
  (document.querySelector("[data-counter]").textContent = JSON.parse(
    localStorage.getItem(key)
  ).length);

export const elementToHTML = (array) =>
  array.map(({ classList, outerHTML }) => {
